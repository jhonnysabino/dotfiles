...
def main():
    ...
    parser.add_argument("--token-file", default=os.getenv("UMAMI_TOKEN_FILE", os.path.expanduser("~/.umami_token")))
    ...
    # For self-hosted, read token from file if no API key provided
    api_key = args.api_key
    if args.deployment == "self-hosted" and not api_key:
        token_file = args.token_file
        if os.path.exists(token_file):
            api_key = open(token_file).read().strip()
        else:
            print(f"Missing token. Set UMAMI_API_KEY or ensure {token_file} exists (run umami_login.py).", file=sys.stderr)
            sys.exit(2)
    elif not api_key:
